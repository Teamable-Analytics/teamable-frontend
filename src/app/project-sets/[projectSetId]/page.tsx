...
async function getRawProjectSetsData(): Promise<ApiTeamSetTemplate[]> {
  const projectSetsURL = new URL('/api/v1/teamset-templates', process.env.BACKEND_BASE_URI as string)
...
async function ProjectPage({params, searchParams}: ProjectPageParams) {
  ...
  const displayProjects = allProjects.filter((project: Project) => project.name.toLowerCase().includes(sidebarSearchTerm.toLowerCase()))
